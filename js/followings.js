...
function setFollowing(userInfo) {
  ...
  if (Object.keys(userInfo).length !== 0) {
    ...
    emptyfollowItem.innerHTML = `<span>팔로우한 계정이 없습니다.</span><div class="user-email">유저를 검색해 팔로우 해보세요!</div>`;
...
async function getFollowing() {
  ...
  try {
    const res = await fetch(`${url}/profile/${accountName}/following?limit=100`, init);
